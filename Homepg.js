...
if (fileComplaintBtn) {
  fileComplaintBtn.onclick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('File complaint button clicked -> redirecting to login');
    window.location.href ='login.html'; // or '/auth/login', adjust path as needed
  };
}
...
// ===== Center the existing permission box at page open =====
document.addEventListener('DOMContentLoaded', () => {
  const centerModal = document.getElementById('permissionCenterModal');
  const panelHost = document.getElementById('permissionPanelHost');
  const permCenterGrant = document.getElementById('permCenterGrant');
  const permCenterDeny = document.getElementById('permCenterDeny');

  // Grab the existing permission modal content from the page
  const originalPermissionModal = document.getElementById('permissionModal'); // present in Homepg.html
  const originalBody = originalPermissionModal?.querySelector('.modal__body'); // inner content to reuse

  if (centerModal && panelHost && originalPermissionModal && originalBody) {
    // Inject a clone of the existing body content into the centered host
    panelHost.innerHTML = '';
    panelHost.appendChild(originalBody.cloneNode(true));

    // Hide the original top-left (or off-screen) modal completely so only the centered one shows
    originalPermissionModal.classList.add('hidden');

    // Show the centered version
    centerModal.classList.remove('hidden');
  }

  // Close on backdrop
  centerModal?.querySelector('.modal__backdrop')?.addEventListener('click', () => {
    centerModal.classList.add('hidden');
  });

  // Deny: simply close the centered modal
  permCenterDeny?.addEventListener('click', () => {
    centerModal.classList.add('hidden');
  });

  // Grant: run permission flow, then close and permanently remove original
  permCenterGrant?.addEventListener('click', async () => {
    await requestHomeLocationPermission();
    centerModal.classList.add('hidden');
    originalPermissionModal?.remove();
  });
});

// Minimal location permission wrapper
function requestHomeLocationPermission() {
...
// === Login popover (below Login/Signup button) ===
const loginBtn = document.getElementById('loginBtn');
const rolePopover = document.getElementById('roleChoicePopover');
const adminLoginBtn = document.getElementById('adminLoginBtn');
const citizenLoginBtn = document.getElementById('citizenLoginBtn');

function openRolePopover() {
  rolePopover?.classList.remove('hidden');
  // trap focus start
  setTimeout(() => adminLoginBtn?.focus(), 0);
}
function closeRolePopover() {
  rolePopover?.classList.add('hidden');
}
...
loginBtn?.addEventListener('click', (e) => {
  ...
  const isOpen = !rolePopover.classList.contains('hidden');
  if (isOpen) {
    closeRolePopover();
  } else {
    openRolePopover();
...
document.addEventListener('click', (e) => {
  if (!rolePopover || rolePopover.classList.contains('hidden')) return;
  const within = rolePopover.contains(e.target) || loginBtn.contains(e.target);
  if (!within) closeRolePopover();
});

// Keyboard close
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && rolePopover && !rolePopover.classList.contains('hidden')) {
    closeRolePopover();
    loginBtn?.focus();
  }
});
...
adminLoginBtn?.addEventListener('click', () => {
  window.location.href = 'Adminlogin.html'; // same dash folder
});
citizenLoginBtn?.addEventListener('click', () => {
  window.location.href = 'Citizenlogin.html'; // same dash folder
});
